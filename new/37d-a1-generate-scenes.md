...
```javascript
// Create main list for the book
const BOOK_FOLDER = "[BOOK_FOLDER]"; // e.g., "0028_red_and_black"

await mcp__todoit__todo_create_list({
  list_key: BOOK_FOLDER,
  title: `AI Image Generation for ${BOOK_FOLDER}`,
  list_type: "sequential"
});

// Set book folder property
...
// Add project tag
await mcp__todoit__todo_add_list_tag({
  list_key: BOOK_FOLDER,
  tag_name: "37d"
});
```
...
```javascript
// Create 25 main scene tasks with 4 subtasks each
for (let i = 1; i <= 25; i++) {
  const sceneNum = i.toString().padStart(4, '0'); // e.g., "0001"
  const sceneKey = `scene_${sceneNum}`;
  
  // Add main task for the scene
  await mcp__todoit__todo_add_item({
    list_key: BOOK_FOLDER,
    item_key: sceneKey,
    content: `Generate image using scene_${i.toString().padStart(2, '0')}.yaml`
  });
...
// Add final tasks for audio and video
await mcp__todoit__todo_add_item({
  list_key: BOOK_FOLDER,
  item_key: "audio",
...
3. **Subtasks Created:** Report total count (should be 25 scenes × 4 subtasks + 3 additional = 103 subtasks)
4. **Scene Generation Status:** All scene_gen subtasks marked as completed
5. **Ready for Next Stage:** Confirm system is ready for 37d-a2 (style application)

The system is now ready for the next agent (37d-a2-apply-style) to process the scene_style subtasks.