...
NIE używaj playwright-show-browser
...
### 1) Pobierz następne zadanie do pobrania (lub użyj podanego task_key)

```javascript
let sceneKey, threadId;

// Sprawdź czy task_key został podany na wejściu
const taskKeyMatch = input.match(/Task key:\s*(\S+)/);
if (taskKeyMatch) {
  // Użyj podanego task_key
  sceneKey = taskKeyMatch[1];
  console.log(`Używam podanego task_key: ${sceneKey}`);
  
  // Pobierz thread_id dla tego konkretnego zadania
  ...
  threadId = threadIdProperty.property_value;
} else {
  // Szukaj następnego zadania automatycznie
  console.log("Brak task_key - szukam następnego zadania...");
  
  // Pobierz wszystkie properties zadań z statusem pending/in_progress
...
// Sprawdź czy przycisk download istnieje i pobierz
const downloadExists = await mcp__playwright-headless__browser_evaluate({
  function: `() => {
    const buttons = Array.from(document.querySelectorAll('button'));
    return buttons.some(b => (b.getAttribute('aria-label')||'').includes('Download this image'));
  }`
});
...
let moved;
do {
  // Spróbuj cofnąć do pierwszej odpowiedzi
  ...
  // Sprawdź czy w tej odpowiedzi jest dostępny download i pobierz
  if (moved) {
    const downloadExists = await mcp__playwright-headless__browser_evaluate({
      function: `() => {
        const buttons = Array.from(document.querySelectorAll('button'));
...
move_one() {
  src="$1"; name="$2"; idx=0
  suffixes=("" "_a" "_b" "_c" "_d" "_e" "_f" "_g" "_h" "_i" "_j" "_k" "_l" "_m" "_n")
  out="${DEST_DIR}/${name}.png"
  
  while [ -e "$out" ]; do 
    idx=$((idx+1))
    out="${DEST_DIR}/${name}${suffixes[$idx]}.png"
  done
...
### Monitoring stanu zadań (przykłady)
...
### Debugowanie właściwości zadań (przykłady)

```javascript
// Pobierz wszystkie właściwości jednym wywołaniem
const allItemsProperties = await mcp__todoit__todo_get_all_items_properties({
  list_key: "0037_wuthering_heights"
});
