#!/usr/bin/env python3
"""Test script for 37d-c4-todoit download functionality"""
...
def download_images_test(book_folder: str):
    """Test download images functionality using direct MCP calls"""
    ...
    try:
        # Get list properties using direct MCP calls
        console.print(f"[yellow]Getting properties for source list: {source_list}[/yellow]")
        
        # Get list properties directly - these calls should work in Claude Code environment
...
def download_from_chatgpt(project_id: str, thread_id: str, scene_key: str, book_folder: str):
    """Download images from a specific ChatGPT thread"""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        console.print("[red]Usage: python test_download.py <book_folder>[/red]")
        console.print("[yellow]Example: python test_download.py 0009_fahrenheit_451[/yellow]")
        sys.exit(1)
    
    result = download_images_test(sys.argv[1])
