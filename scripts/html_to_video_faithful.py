...
Cel: wierne zgrywanie od t=0. Dwa tryby: klatkowy (domyślny) i CZAS RZECZYWISTY (--realtime).

Kluczowe opcje:
--realtime               nagrywaj realtime przez Playwright (WebM → MP4). Likwiduje przyspieszenia animacji.
--channel chrome         uruchom systemowego Chrome (rendering jak „na żywo”).
--reload                 przeładuj tuż przed startem nagrywania.
--restart-css            zresetuj animacje CSS.
--restart-waapi          zresetuj Web Animations API.
--wait-fonts             czekaj na document.fonts.ready.
--ready-selector CSS     czekaj aż selektor będzie widoczny.
--wait-until X           domcontentloaded|load|networkidle dla goto/reload.
--log-console            loguj console i błędy strony.
