#!/usr/bin/env python3
"""
Script to validate gpt5-book_styles.csv (no header format)
"""
...
def main():
    ...
    # Read CSV without header and validate
    csv_books = set()
    csv_styles = set()
    issues = []
    
    with open(csv_file, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)  # No DictReader, just plain reader
        for row_num, row in enumerate(reader, 1):
            if len(row) != 2:
                issues.append(f"Row {row_num}: Invalid format (expected 2 columns, got {len(row)})")
                continue
