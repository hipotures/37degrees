#!/bin/bash
# TODOIT CLI wrapper for reading next Deep Research task
# Usage: ./todoit-read-research-task.sh
# Output: JSON with all needed data for Deep Research generation
...
TASK_JSON=$(python "$PROJECT_ROOT/scripts/internal/find_next_research_task.py" 2>/dev/null)
PYTHON_EXIT=$?

if [ $PYTHON_EXIT -ne 0 ]; then
  echo '{"error": "Failed to execute find_next_research_task.py"}' >&2
  exit 1
fi
...
if [ -z "$STATUS" ]; then
  echo '{"error": "Invalid JSON from find_next_research_task.py"}' >&2
  exit 1
fi
