...
@app.get("/api/lists")
async def get_all_lists(limit: int = 50, offset: int = 0, search: str = "", favorites_only: bool = False):
    ...
    try:
        ...
        for list_obj in all_lists:
            ...
            # Count by status
            pending_count = sum(1 for item in items if str(getattr(item, 'status', 'pending')) == 'pending')
            in_progress_count = sum(1 for item in items if str(getattr(item, 'status', 'pending')) == 'in_progress') 
            completed_count = sum(1 for item in items if str(getattr(item, 'status', 'pending')) == 'completed')
            failed_count = sum(1 for item in items if str(getattr(item, 'status', 'pending')) == 'failed')
        ...
        filtered_lists.sort(key=lambda x: x['updated_at'], reverse=True)
...
async def get_list_details(list_key: str):
    ...
    try:
        ...
        try:
            ...
            if favorite_prop.get('property_value') == 'true':
                is_favorite = True
        ...
        return {
            ...
            "list": {
                ...
                'description': list_obj.description,
                'status': str(list_obj.status),
                'list_type': str(list_obj.list_type),
                'is_favorite': is_favorite,
                'properties': properties,
                'created_at': str(list_obj.created_at),
...
async def toggle_favorite(list_key: str):
    ...
    try:
        ...
        try:
            ...
            is_currently_favorite = current_prop.get('property_value') == 'true'
        except:
...
async def get_list_properties(list_key: str):
    """Get all properties for a specific list"""
    try:
        mgr = get_manager()
        
        # Get list info to verify it exists
        lists = mgr.list_all()
        list_obj = next((l for l in lists if l.list_key == list_key), None)
        if not list_obj:
            raise HTTPException(status_code=404, detail=f"List '{list_key}' not found")
        
        # Get all properties - we'll need to implement this differently
        # For now, let's get known properties one by one
        properties = {}
        ...
        for prop_key in known_props:
            try:
                result = mgr.get_list_property(list_key, prop_key)
                # TodoManager returns string value directly, or raises exception if not found
                if result and isinstance(result, str):
                    properties[prop_key] = result
            except Exception as e:
...
async def get_item_properties(list_key: str, item_key: str):
    ...
    try:
        ...
        # Get all common item properties
        properties = {}
        known_props = ['priority', 'category', 'assignee', 'deadline', 'tags', 'notes', 'difficulty']
        for prop_key in known_props:
            try:
                result = mgr.get_item_property(list_key, item_key, prop_key)
                # TodoManager returns string value directly
                if result and isinstance(result, str):
                    properties[prop_key] = result
            except:
...
async def get_subitem_properties(list_key: str, item_key: str, subitem_key: str):
    """Get all properties for a specific subitem"""
    try:
        mgr = get_manager()
        
        # Get all common subitem properties
        properties = {}
        known_props = ['priority', 'assignee', 'deadline', 'notes', 'difficulty', 'estimated_time']
        for prop_key in known_props:
            try:
                result = mgr.get_item_property(list_key, subitem_key, prop_key, parent_item_key=item_key)
                # TodoManager returns string value directly
                if result and isinstance(result, str):
                    properties[prop_key] = result
            except:
